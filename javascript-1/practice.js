...
// two parameters and return a string that says "NAMEPARAM loves LOVEPARAM" with the appropriate parameters in the 
// string. e.g. "Joseph loves music"
...
// and check to see if the number in the array is GREATER than 100. If it is, push 'big' as a string to the answers 
// array. If the number is LESS than or EQUAL to 100, push 'small' as a string to the answers array. Return the answers
// array inside of the function.
...
const total = myNumbers.reduce(reducer) 

//////////////////PROBLEM 17////////////////////

// Finally, use .forEach to find the index of each item in the array. To begin, create an empty array called 'myNumbersIndex'. Then, use
// forEach to push each item's index from the myNumbers array into the newly created myNumbersIndex array. 
const myNumbersIndex = [];

function iterate(elem, index) {
  myNumbersIndex.push(index)
  
return myNumbers.forEach(iterate);
}
// Code Here
...
var forTheLoveOfGeorge = notGeorge.map( x => 'George')
...
for(var i=0; i<enemies.length; i++) {
	...
	var enemies = people.filter(people => friend = false)	
}
